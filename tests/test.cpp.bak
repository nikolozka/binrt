...
void delay(){
	...
	if(curr_az<13){
		del_l = floor((idt[curr_az][curr_el] / (MY_TYPE)1000) * (MY_TYPE)SAMPLE_RATE);
		del_r = 1;
	}
	else{
		del_r = floor((idt[curr_az][curr_el] / (MY_TYPE)1000) * (MY_TYPE)SAMPLE_RATE);
		del_l = 1;
	}
	...
	while(std::getline(fin, line)){
		std::stringstream ss(line);
			...
			f++;
        		...
        		idt[k][j] = strtod(line.c_str(), NULL);
			j++;
			if (j == 50){
				j = 0;
				k++;
